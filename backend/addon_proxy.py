...
class AITweaker:
    def __init__(self):
        ...
        self.gemini_html_pattern = re.compile(r'^https?:\/\/gemini\.google\.com\/.*', re.S)
...
"""
    ...
    def modify_gemini_html(self, flow: http.HTTPFlow) -> None:
        ...
        try:
            ...
            ctx.log.info(f"Inspecting potential Gemini HTML: {flow.request.url}")

            # CSP bypass: Strip Content-Security-Policy to allow inline script injection
